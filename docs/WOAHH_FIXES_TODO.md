...
**Legend:** `[x]` = done (where it lives is noted on the item) · `[ ]` = open. ⚠️ Most ticked items are **built on branch `feat/overnight-fixes-2026-06-11` (pushed, NOT merged to main/prod)** — they ship when the founder reviews + merges. Full per-item detail: `docs/MORNING_REPORT_2026-06-11.md`.
...
- [x] **2. "Get set up" section auto-clears done steps.** Each setup step disappears (or collapses as ✓ done) once completed, so the section only ever shows what's left. **✅ BUILT on `feat/overnight-fixes-2026-06-11` — awaiting founder review/merge.**
...
- [x] **4. Declutter the sidebar.** Grouped/collapsible nav **✅ BUILT on `feat/overnight-fixes-2026-06-11`** — but see open follow-up below.
  - [ ] **OPEN follow-up (founder feedback 2026-06-11):** the grouped/dropdown version is **too spaced out** — tighten the vertical rhythm: smaller gaps between groups and items, denser dropdown padding, so more nav fits on screen without scrolling.
- [x] **5. Obvious "setting up & domain" checklist.** A prominent checklist that walks the merchant through setup including their `<slug>.woahh.app` domain/storefront go-live. **✅ BUILT on `feat/overnight-fixes-2026-06-11` — awaiting founder review/merge.**
- [x] **6. Disable delivery everywhere, temporarily.** Hide/disable delivery across storefront, checkout, dashboard, KDS, marketing copy (keep courier code dormant behind a feature flag — carries over old TODO 6.1). **✅ BUILT on `feat/overnight-fixes-2026-06-11` — awaiting founder review/merge.**
- [x] **7. Storefront only viewable live once the merchant publishes.** Unpublished storefronts must not be publicly reachable. **✅ BUILT on `feat/overnight-fixes-2026-06-11` — awaiting founder review/merge.**
- [ ] **8. On publish, put it on the `<slug>.woahh.app` domain.** Publishing is what activates the merchant's subdomain.
- [x] **9. Limit storefront changes.** Rate-limit / restrict how much the merchant can fiddle with the storefront (template/branding churn) so they can't endlessly play with the web presence. **✅ BUILT on `feat/overnight-fixes-2026-06-11` — awaiting founder review/merge.**
- [x] **10. Fix checkout timings — show proper live timings.** Pickup/prep time shown at checkout must be real and live, not placeholder. **✅ BUILT on `feat/overnight-fixes-2026-06-11` — awaiting founder review/merge.**
...
- [x] **13. Pay at venue or pay online — merchant-configurable.** Merchant setting that controls which payment options the customer sees at checkout. **✅ BUILT on `feat/overnight-fixes-2026-06-11` — awaiting founder review/merge.**
...
- [x] **15. Checkout offers customer account before guest.** Flip the order: sign-in/create-account is the primary path, guest checkout secondary. *(Founder item 16.)* **✅ BUILT on `feat/overnight-fixes-2026-06-11` — awaiting founder review/merge.**
- [x] **16. Smarter automatic usernames.** Auto-generated usernames get numeric suffixes (e.g. `priya`, `priya2`, `priya17`) instead of failing or producing ugly handles. *(Founder item 17.)* **✅ BUILT on `feat/overnight-fixes-2026-06-11` — awaiting founder review/merge.**
- [x] **17. QR codes accessible without enabling dine-in.** A merchant should be able to generate/use QR codes even when dine-in is off. *(Founder item 18.)* **✅ BUILT on `feat/overnight-fixes-2026-06-11` — awaiting founder review/merge.**
...
- [ ] **26. Bring back the merchant Analytics page — it's gone missing.** The analytics dashboard exists (7 widgets: revenue trend, fulfillment mix, top products, peak hours, customers, categories, marketing — shipped earlier) but merchants can't see it anymore. Find why it vanished — most likely casualty of the sidebar regroup (#4) dropping the nav link, or a route/tier-gate change — and restore it with a visible sidebar entry. Add a regression check: every dashboard page must be reachable from the nav. *(Added 2026-06-11.)*
