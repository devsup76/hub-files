...
- **✅ Online card payments now work on published templates** (Option B, `8e4b7e2`+`b9f5271`): the bespoke `PublishedStorefront` checkout takes cards via the same Stripe flow as the default (gated by `online_card_enabled` + server-side `charges_enabled`), with a context-aware Payment step (Pay-by-card / Pay-at-venue). No need to stay on the default storefront for online cards.
...
- ~~KEY GAP — bespoke template takes orders but not online cards~~ — **CLOSED (Option B, commits `8e4b7e2` + `b9f5271`).** The bespoke `PublishedStorefront` checkout now takes **online card payments**, reusing the default's exact Stripe flow (`CardPaymentDialog` + `stripe-payment-intent` + C1 server total + manual-capture-on-confirm) under the SAME gate (`online_card_enabled` && `charges_enabled` (server) && !dine_in). The Payment step is context-aware: **Pay-by-card** panel when online cards are on, **Pay-at-venue** panel otherwise, decorative showcase only in preview/demo. Adversarial-reviewed (correctness PASS / preview APPROVE) + smoke-verified live (pay-at-venue panel, no fake fields). **Premium template + online cards now works** — the live card E2E (real Stripe test card) happens in Phase 9 with the merchant's connected account.
