# Growerr — Additional Legal Obligations: Retail/Grocery & Events Expansion
...
Before diving into verticals, the user has confirmed: **money flows directly from customer to merchant/organiser via their own Stripe account. Growerr never holds or touches transaction funds.** This is the correct structure and eliminates the largest class of additional obligations (AFSL, AUSTRAC, trust accounting). All analysis below assumes this model holds.

Growerr charges a separate platform/booking fee — that fee is Growerr's revenue, subject to GST, and is the only money Growerr ever receives.
...
**Who is responsible:** The merchant (grocery/retailer) is legally responsible for labelling compliance. However, if Growerr's platform displays product listings without country of origin information and this results in a customer being misled, Growerr could be exposed to ACL s18 (misleading conduct) liability.

**What Growerr must do:**
...
**Why this matters for Growerr:** If a customer has an allergic reaction and Growerr's platform displayed inaccurate allergen information provided by the merchant, Growerr could face a claim. A platform-level disclaimer and a requirement in the Merchant Terms shifts that liability clearly to the merchant.
...
Most small independent grocers on Growerr will be below the threshold. However, if a larger grocery chain signs up, they may have unit pricing obligations. Growerr's Merchant Terms should require merchants to comply with all applicable pricing display laws.

### A.4 Consumer Guarantees on Goods (Different From Services)

The *Australian Consumer Law* treats goods and services differently. For goods purchased through Growerr's platform (groceries, retail products):
...
If Growerr's delivery integration (Uber Direct, etc.) is used for grocery/perishable delivery:
- Merchant Terms must require merchants to comply with food safety laws during delivery
- Growerr should disclaim liability for food safety failures that occur during the delivery leg
...
- Growerr's checkout must include an age confirmation step for alcohol orders
- The delivery driver (via Uber Direct or equivalent) must also verify age at the door — Growerr's Merchant Terms must require merchants to communicate this to their delivery provider
...
- **Recommendation:** Do not permit nicotine vaping product sales through Growerr without specific legal advice — the regulatory landscape is rapidly changing and exposure is high
...
- **Recommendation:** Prohibit pharmacy-only or prescription items from Growerr product listings unless the merchant is a registered pharmacy operating compliantly
...
- Growerr should not auto-calculate prices by weight — leave this to the merchant's system to ensure accuracy
...
**If an event is cancelled:** The consumer is entitled to a **full refund** of the ticket price. This is a statutory right — no terms and conditions can override it. The refund obligation falls on the event organiser (who received the money directly). However, Growerr's platform fee may also be refundable if the service (booking facilitation) was not ultimately delivered in a meaningful way.
...
- Growerr's platform must display the organiser's refund policy on the event listing page
- Growerr's own platform fee policy on cancellation must be stated (does Growerr refund its fee if an event is cancelled? Recommended: yes — the failure is the organiser's, not the customer's)
...
- If Growerr adds any secondary market/resale functionality in future, it must be geofenced and price-capped to comply with each state's rules
- Consider whether the platform itself needs to register as a "ticket seller" under any state legislation — this depends on how Growerr facilitates the sale

### B.3 Holding Advance Ticket Proceeds (Trust/Float Risk)

The user confirmed money goes directly from customer to organiser. This is the correct approach and eliminates most exposure. However, confirm that Stripe's payment flow in the event context does not create any holding period where Growerr intermediates funds even temporarily (e.g., Stripe Connect holds in a platform account before sweeping to the organiser). If it does, trust accounting or AFSL issues arise.

**Do not add:**
- A "Growerr holds your ticket funds until the event" model — this is trust accounting territory
...
If Growerr wants to offer this in future: partner with a licensed insurer who holds the AFSL and refer customers to them rather than selling directly through the platform.

### B.5 Event Organiser Public Liability Insurance

Event organisers are responsible for public liability at their events. A patron injured at a venue can sue the organiser. Growerr is not the organiser, but:

**Growerr's Merchant Terms must require event organisers to:**
...
Growerr should consider requiring organisers to provide evidence of public liability insurance before their event listing goes live.
...
**Growerr's obligations:** Merchant Terms must require event organisers to comply with all applicable WWCC laws. Growerr does not need to verify checks itself, but must not knowingly facilitate an event that is obviously non-compliant.
...
This is the industry norm and is consistent with Ticketek and Humanitix's models. Growerr must display the "no general cooling-off" position clearly before purchase alongside the organiser's specific policy.

### B.8 Misleading Event Descriptions (ACL s18)

Event listings that misrepresent the event expose both the organiser and Growerr to ACL s18 liability. Specific risks:
...
- Growerr should have a flagging/complaints mechanism for event listing errors
- Growerr's platform should not algorithmically generate or auto-fill any event descriptions — leave all content to the organiser

### B.9 Charitable and Not-For-Profit Events

Many events are fundraisers (charity galas, community events, school fundraisers). If Growerr facilitates ticket sales for a charitable fundraising event, state-based fundraising laws come back into play (as discussed in `legalities.md` Section 7).

**Key issue:** If Growerr's platform is used to sell tickets where proceeds go to a charity, Growerr may be considered a "fundraising platform." This potentially requires Growerr to be registered as a fundraiser in each applicable state — or at minimum, the event organiser must hold the appropriate state fundraising authority.

**Growerr's Terms must:** require event organisers running charity events to hold all applicable state fundraising registrations and authorities before using the platform.
...
None of these are Growerr's responsibility — they fall on the event organiser. However, Merchant Terms must make clear that the organiser is responsible for all such permits, and Growerr reserves the right to remove listings for events that lack required approvals.
...
Customer → Merchant/Organiser (primary liability)
         → Growerr (secondary, if platform contributed to the harm or misrepresentation)
```

Growerr minimises its position in this chain by:
...
**Events:** Event attendees may include information about religious observances (e.g., a church event), political activity (fundraiser for a candidate), or sexual orientation (Pride events). This is **sensitive information** under the Privacy Act. Growerr's role is to process this data on behalf of the organiser — the DPA framework already covers this, but the Privacy Policy must acknowledge that event-related sensitive information may be processed.
...
- Growerr's booking/platform fee is also subject to 10% GST — Growerr accounts for this
...
- Refunds: when a ticket is refunded, both the organiser and Growerr must issue a tax adjustment note (the GST equivalent of a credit note) — the platform must support this
...
Since money goes directly to the merchant/organiser via their own Stripe account, chargebacks hit the merchant/organiser directly. However, Growerr's Merchant Terms must:
...
- Give Growerr the right to suspend a merchant's listing if they have a pattern of unresolved disputes
...
**What is eliminated:**
- AFSL obligations (Growerr is not providing a financial service)
- AUSTRAC obligations (Growerr is not handling payment flows)
- Trust accounting (Growerr never holds funds)
...
When an event organiser cancels an event and refuses or is unable to refund tickets (e.g., they've already spent the money and go insolvent), the customer's recourse is against the organiser — not Growerr. Growerr is not legally obligated to cover these refunds. However:
- The customer will associate the loss with Growerr's platform
- ACCC has historically pressured platforms to step in for consumers even when not legally required
- Consider whether Growerr wants a voluntary "buyer protection" policy for events (funded by a small event risk fee) — this is a commercial decision, not a legal one

**Growerr's booking/platform fee:**
If an event is cancelled and Growerr refunds its own platform fee (recommended), this is straightforward. If Growerr does not refund it, there is a credible ACL consumer guarantee argument that the booking service was not ultimately provided. **Recommend: refund the platform fee on any event cancellation.**
...
| Obligation | Who it falls on | Growerr's role |
|---|---|---|
...
| Age verification for alcohol/tobacco | Merchant (primary) + Growerr (checkout) | Add checkout age confirmation step |
...
| Sensitive dietary data (Privacy Act) | Growerr as processor | Update Privacy Policy |
...
| Anti-scalping compliance | Organiser + Growerr (platform) | Prohibit in AUP; no secondary market without price cap |
| No ticket insurance without AFSL | Growerr | Do not offer — refer to licensed insurer only |
...
| Misleading event descriptions | Organiser (primary) + Growerr | Do not auto-generate content; complaints mechanism required |
...
| GST adjustments on refunds | Growerr (for its fee) + Organiser | Support tax adjustment notes in platform |
| Sensitive event attendance data (Privacy Act) | Growerr as processor | Update Privacy Policy |
