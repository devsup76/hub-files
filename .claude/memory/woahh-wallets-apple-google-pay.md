...
Apple Pay + Google Pay on the customer web checkout, 2026-06-12. **MERGED to `main` + LIVE on woahh.app** (founder explicitly authorized wallets→main; the app repo otherwise stays branch→founder-review). Design + 23-risk register in `docs/APPLE_GOOGLE_PAY_RISK_AND_DESIGN.md` (now carries a 2026-06-12 "DOUBLE-CHECK — verified status" section). Founder's research doc `docs/APPLE_GOOGLE_PAY_PLAN.md` is fallible (its "zero server changes" claim was wrong).
