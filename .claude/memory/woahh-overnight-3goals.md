...
2. **Website UI upgrade** — branch `feat/marketing-home-redesign` off main (worktree `repo-home`). **✅ DONE (pending founder pick) 2026-06-07.** 3 distinct directions — **Momentum** (bold conversion-SaaS), **Warmth** (editorial/charity-led), **Clarity** (product-led Linear/Notion, dark-mode native) — each across **home + /eat marketplace + merchant dashboard**, non-destructive preview at `/home-preview` (live untouched). Commits `408b904`→`21c9f01` (pushed). All screenshot-verified distinct + industry-level; tsc clean, build green. Gotcha: local `vite preview` can serve stale dist (false 404s) — Cloudflare builds fresh.
