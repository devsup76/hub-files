...
**STAGED BACKEND — ✅ DEPLOYED/RUN 2026-06-13:** (1) migration `20260612210000` run live by founder; (2) deployed live: `refund-order` **v20** + `order-notify` **v31** (both `verify_jwt=true` preserved, smoke-tested gateway-401). staff-pin-login unchanged. **ONLY remaining: merge `feat/founder-fixes-2026-06-12` → `main` when the founder is happy with the preview.** (Deploy token was the `sbp_…` in /tmp/sb-token — still valid 2026-06-13; rotate when done.)
