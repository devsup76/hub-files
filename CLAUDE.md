...
See `/workspaces/GrowthHub/docs/WOAHH_FIXES_TODO.md` for the current punch list — **rewritten 2026-06-10 from the founder's 17-item fixes list** (onboarding checklist + ABN payment gate, sidebar declutter, publish-gated storefronts on `<slug>.woahh.app`, live checkout timings, pay-at-venue config, charity money-flow fix, QR-without-dine-in, etc.) plus a verified carried-over backlog. The pre-2026-06-10 punch list is in git history.

**Docs reorg 2026-06-12:** `docs/` is now foldered — `architecture/` (designs/specs), `launch/` (go-live + onboarding + checklists), `security/` (audits + hardening), `schema/` (live DB snapshots + drift), `sql/` (one-off founder-run bundles), plus the existing `business/ legal/ hardware/ pitch/`. `docs/README.md` is the index; `docs/WOAHH_FIXES_TODO.md` stays at root. Removed (recoverable in git history): the duplicate Apple-Pay `.well-known` scratch copy, `WALLETS_PREP.md` (superseded by the design doc) + its spent `WALLETS_RUN_THESE.sql`, the done dated handoffs (`ADVISORY_`/`OVERNIGHT_PLAN_`/`SECURITY_OVERNIGHT_REPORT_`/`MORNING_REPORT_2026-06-11`, `MORNING_REPORT_2026-06-13`, `MERCHANT_DEMOS_TASKS`), and the `docs/proof/*` card-form screenshots. **Kept** the `sql/*RUN_THESE.sql` bundles — `WOAHH_FIXES_TODO.md` lists running `FOUNDER_RUN_THESE.sql` + `SECURITY_FIXES_RUN_THESE.sql` as OPEN founder side-quests, so they are NOT spent.

**Docs cleanup 2026-06-10:** an adversarially-verified audit deleted the completed one-off SQL bundles (`FOUNDER_RUN_THESE.sql`, `FOUNDER_RUN_NEXT.sql`, `ALIGN_SCHEMA_DRIFT.sql`, `FIX_DRIFTED_RPCS.sql` — **all confirmed run on the live DB**; re-running FOUNDER_RUN_NEXT would actually clobber the drift hotfix), the done handoffs/run logs (`FIRST_MERCHANT_LAUNCH.md`, `MORNING_REPORT_2026-06-10.md`, `OVERNIGHT_SQUARE_PLAN.md`, `POST_SQUARE_TODO.md`, `AI_ARCHITECTURE.md`, all of `docs/archive/`), and Lovable-era / deferred-retail / dead-pricing docs (`stripe.md`, `SHOP_LOVABLE_PROMPTS/RESEARCH`, `SHOP_HARDWARE`, `expansion-verticals`, co-founder pitch trio). Everything is recoverable from git history. Current launch references:
- **First-merchant onboarding:** `/workspaces/GrowthHub/docs/launch/FIRST_MERCHANT_READINESS.md` (GO/NO-GO + hard gates) + `/workspaces/GrowthHub/docs/launch/MERCHANT_ONBOARDING_RUNBOOK.md` (dependency-ordered zero-to-live runbook). All go-live migrations (guest-consent, C1, anon-trigger guard, Square additive + audit-fix + drift hotfixes) are **applied to live** (`pmnyhbhtkcfoozkinieo`) and the full online flow was **verified 11/11 live on 2026-06-10**.
- **Square (sandbox → live):** `/workspaces/GrowthHub/docs/architecture/SQUARE_POS_INTEGRATION.md` (architecture + API refs) + `/workspaces/GrowthHub/docs/launch/SQUARE_PRODUCTION_CHECKLIST.md` (sandbox→production checklist; sandbox authorize→capture→refund verified live on test-bistro). Square needs an AU Square dev account + AU bank + PAAF PDS/FSG review before live. **Pending revert:** test-bistro is still flipped to Square sandbox — `docs/sql/SQUARE_SANDBOX_GOLIVE.sql` holds the revert block (READINESS gate B.10 needs it before a real Stripe test order).
- **Audit findings + deferred items:** `/workspaces/GrowthHub/docs/security/AUDIT_FINDINGS_2026-06-09.md` — money/correctness/payment-race issues found + fixed; **staged-for-decision items not yet done:** H-2 (release committed stock on card-payment failure — inventory-DoS guard), Square go-live compliance, and `payment_provider`/`square_payment_ready` write-boundary hardening.
