...
- **First-merchant onboarding:** `/workspaces/GrowthHub/docs/FIRST_MERCHANT_READINESS.md` (GO/NO-GO + hard gates) + `/workspaces/GrowthHub/docs/MERCHANT_ONBOARDING_RUNBOOK.md` (dependency-ordered zero-to-live runbook). All go-live migrations (guest-consent, C1, anon-trigger guard, Square additive + audit-fix + drift hotfixes) are **applied to live** (`pmnyhbhtkcfoozkinieo`) and the full online flow was **verified 11/11 live on 2026-06-10**.
- **Square (sandbox → live):** `/workspaces/GrowthHub/docs/SQUARE_POS_INTEGRATION.md` (architecture + API refs) + `/workspaces/GrowthHub/docs/SQUARE_PRODUCTION_CHECKLIST.md` (sandbox→production checklist; sandbox authorize→capture→refund verified live on test-bistro). Square needs an AU Square dev account + AU bank + PAAF PDS/FSG review before live. **Pending revert:** test-bistro is still flipped to Square sandbox — `docs/SQUARE_SANDBOX_GOLIVE.sql` holds the revert block (READINESS gate B.10 needs it before a real Stripe test order).
