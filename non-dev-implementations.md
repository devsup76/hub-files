...
This removes the "go buy your own hardware" barrier while protecting woahh's cash. You don't gift hardware — you extend the startup fee to cover a lease, which is a business cost recoverable through LTV.
...
Pre-configured before shipping: woahh PWA as home screen, Stripe Terminal paired, dashboard auto-launches on boot.
...
woahh's `/dashboard/kitchen` route is a webpage. Any screen with a browser can become a KDS. An Android TV stick (~$40–60 bulk) plugs into the HDMI port of whatever TV or monitor the merchant already has on the wall — or a cheap one they source themselves. The stick runs **Fully Kiosk Browser for Android TV** (same $7 USD one-time licence), locked to the kitchen URL.
...
3. Set start URL: `https://app.woahh.com/dashboard/kitchen`
...
3. Log in once with woahh credentials
...
**Packaging:**
- Branded unboxing experience: woahh-branded outer box, welcome card, QR code setup guide
...
**Returns / damage:**
- 12-month hardware warranty covered by woahh (sourced from supplier warranty)
...
- Draft two documents: a **Merchant Terms of Service** (the contract between woahh and business owners) and a **Customer Privacy Policy** (the notice for end consumers)
...
- If you have EU/UK customers, GDPR applies regardless of where woahh is incorporated — include lawful basis for each processing activity
...
- The agreement must explicitly disclose:
  - The woahh platform fee (0.15% of GMV to charity, 0.15% to woahh)
  ...
  - That woahh can suspend the account for policy violations
...
- woahh should provide guidance (in the Operations settings or a help article) reminding merchants of this obligation — it is the merchant's responsibility, not woahh's, but woahh facilitating non-compliant pricing creates reputational risk
...
- Register woahh for GST if annual turnover exceeds or is expected to exceed $75,000 AUD

**Watch out for:**
- Subscription fees woahh charges to merchants are also subject to GST — include GST in pricing or clearly state prices are ex-GST
...
**What needs to happen:**
- Confirm woahh's scope: because all card data is handled by Stripe Elements (iframed) and Stripe Terminal (end-to-end encrypted), woahh qualifies for **SAQ-A** (the simplest compliance tier — self-assessment only, no penetration testing required)
...
- Establish a process for handling spam complaints escalated by Clicksend/Resend to woahh's account — complaints above a threshold (0.1% for email) need intervention
...
- Appoint a point of contact for privacy requests (can be a founder email like `privacy@woahh.com`)
...
**What needs to happen:**
- woahh's Merchant Terms must explicitly state that merchants selling age-restricted goods are responsible for compliance with their liquor licence conditions
- woahh should not (and legally cannot) guarantee age verification beyond a self-declaration checkbox — make this limitation clear in the terms
- If a merchant is selling alcohol for delivery, they need a **liquor licence with a delivery endorsement** in their state — woahh should require merchants to confirm this during onboarding for alcohol categories
...
**What needs to happen:**
- **Public liability insurance** — covers claims from merchants or customers arising from woahh's platform (e.g. a bug causes an order to be incorrectly processed)
...
**What needs to happen:**
- Set up a support email (`support@woahh.com`) with a ticketing system — Intercom, Freshdesk, or even a well-managed Gmail at early stage
- Define SLAs: critical issues (orders not processing, payment failures) → 2 hour response; general queries → 24 hours
- The Feedback page (now built in the dashboard) feeds into woahh's internal backlog — establish a triage process for acting on it
